...
Eq4to7_fx <- function(H, M_HC, I_unlabeled, m, V, lambda_vec, maskedValues){
	
	# Equation 4
	if(is.matrix(H)==F){H <- matrix(I_unlabeled,nrow=1)}
	...
	if(is.matrix(M_HC)==F){M_HC <- matrix(rep(0,sum(V)),nrow=1)}
	C_zk_out <- H_j_out%*% M_HC
	C_zk_out <- C_zk_out[maskedValues] # only values of queried dimension
	return(list(H, mue_ij , M_HC, j_winner, H_j_out, C_zk_out))
}
# Equation 9
eq9_fx <- function(k, humbleTeacher, C_zk_out){
	if(humbleTeacher[k]==1){
		t_k <- max(c(C_zk_out[k], 1))
	}else{
		t_k <- min(c(C_zk_out[k], 0))
	}
	return(t_k)
}
...
trial_fx <- function( # fixed
	...
	I_unlabeled <- I
	I_unlabeled[maskedValues] <- NA
	
	Eq4to7 <- Eq4to7_fx( H=H, M_HC=M_HC, I_unlabeled=I_unlabeled, m=m, V=V, lambda_vec=lambda_vec, maskedValues= maskedValues)
...
block_fx <- function( par, agent, block, stimuli, ResponseSet, structure_name, 
		...
		if(nrow(H)>2){
			mOut <- data.frame(cbind(rep(agent,nrow(H)), M_HC))
			write.table(mOut,"H.txt",append=T)
...
agent_fx <- function( par, agent, nBlocks, termCrit,
	...
	for(block in 1: nBlocks){
		...
		if(blockOutcome$block_performance_df[,"probOfError_block"]==0){errorFreeBlocks <- errorFreeBlocks+1}
